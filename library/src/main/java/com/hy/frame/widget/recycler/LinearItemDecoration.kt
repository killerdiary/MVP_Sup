...
open class LinearItemDecoration : BaseItemDecoration {
    ...
    override fun configureItemOutRect(outRect: Rect, view: View) {
        ...
        if (adapter != null && adapter!!.getItemViewType(viewAdapterPosition) == BaseAdapter.TYPE_ITEM) {
            ...
            if (orientation == LinearLayoutManager.VERTICAL)
                outRect.set(paddingLeft, if (paddingTop > 0 && position == 0) paddingTop else 0, paddingRight, if (paddingBottom > 0 && position == adapter!!.dataCount - 1) paddingBottom + divider else divider)
            else
